...
function EditAssetMutation() {
  ...
  const getAdjustmentAssetValueByID = service.getByID({
    ...
    onSuccess: (res) => {
      ...
      reset({ ...data, id_currency: `${data.id_currency}` });
    },
    onError: (err) => {
  ...
  const updateAdjustmentAssetValue = service.update({
    onSuccess: (res) => {
  ...
  const doSubmit = (data) => updateAdjustmentAssetValue.mutate(getPayload({ ...data, status: 1 }));
  const doSave = (data) => updateAdjustmentAssetValue.mutate(getPayload({ ...data, status: 0 }));
    ...
    <>
      <Col>
        <Row gap="4px" alignItems="center">
          <ArrowLeftIcon onClick={() => Router.push('/asset/adjustment-asset-value')} style={{ cursor: 'pointer' }} />
          ...
          <Row justifyContent="flex-end" alignItems="center" nowrap>
            <Row>
              <Row gap="16px">
                <Button
                  ...
                  onClick={() => setModals({
                    confirmation: {
                      ...
                      onOk: () => Router.push('/asset/adjustment-asset-value'),
                    },
                  })}
                >
                  Cancel
                ...
                >
                  {updateAdjustmentAssetValue.isLoading ? 'Loading...' : 'Save as Draft'}
                </Button>
                <Button
                  size="big"
                  variant="primary"
                ...
                >
                  {updateAdjustmentAssetValue.isLoading ? 'Loading...' : 'Submit'}
                </Button>
              </Row>
            </Row>
          </Row>
        ...
        <Spacer size={20} />
        <FormAdjustmentAssetValue form={form} type="update" />
      </Col>
      {modals?.confirmation && (
        <ModalConfirmation
      ...
      {modals?.alert && (
        <ModalAlert
          ...
          onOk={() => {
            closeModals();
            Router.push('/asset/adjustment-asset-value');
          }}
        />
      )}
    </>
