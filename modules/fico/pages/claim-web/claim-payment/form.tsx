...
export const FormVendorInvoicing = (props: IForm<VendorInvoicingFields>) => {
  ...
  const fields: IField<VendorInvoicingFields>[] = [
    ...
    { id: '', type: '' },
    {
      ...
      datasources: queryMasterVendor.data?.map((v) => ({ id: v.id, value: v.id, label: v.text })),
      onChange: ({ target }) => {
        form.setValue('vdr_name', target.selected?.label);
      },
    ...
    { id: '', type: '' },
    {
      ...
      placeholder: 'Select',
      onSearch: (search, field) => setSearchDropdown({ field, search }),
      datasources: queryMasterPPH.data?.map((v) => ({
        id: v.wtax_code, value: v.wtax_code, label: v.name, ...v,
      })),
      validation: { required: '* required' },
      onChange: ({ target }) => {
        ...
        form.setValue('pph_desc', target.selected?.label);
      },
    },
    {
      id: 'pph_desc',
      type: 'text',
      label: 'PPH Description',
      disabled: true,
      validation: { required: '* required' },
    },
    {
...
const FormExpenseEntry = (props: IForm<VendorInvoicingFields>) => {
  ...
  }, [formVendorInvoicing.getValues('for_acdoca'), formVendorInvoicing.getValues('ppn_code'), formVendorInvoicing.getValues('pph_code')]);
  ...
  const preparePayload = (newItems: ExpenseEntryFields[]) => {
    ...
    const total = newItems.reduce((accumulator, item) => {
    ...
    formVendorInvoicing.setValue('ppn_total', (total * ppnPercentage) || 0);
    ...
    formVendorInvoicing.setValue('pph_total', (total * pphPercentage * pphWtax) || 0);
  };
