...
function CreateAR() {
  ...
  const getPayload = (data: IAccountReceivable) => ({
    ...
    return_number: Number(data.return_number),
    transfer_number: Number(data.transfer_number),
    giro_number: Number(data.giro_number),
    down_payment_number: Number(data.down_payment_number),
    cndn_number: Number(data.cndn_number),
    cndn_num: Number(0),
  });
    ...
    <>
      ...
      {modals?.alert && (
        <ModalAlert
          ...
          onOk={() => {
            closeModals();
            Router.push('/account-receivable/list');
          }}
        />
      )}
    </>
