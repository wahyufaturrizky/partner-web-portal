...
export const FormPO = (props: IForm<DownPaymentFields>) => {
  ...
  const queryMasterPPN = useQueryMasterPPN({
    query: { search: getSearchValue('tax_code') },
  });
  ...
  const fields: IField<DPAccountFields>[] = [
    {
      ...
      onChange: ({ target }) => {
        ...
        const selectedCurrency = formGeneralJournal.getValues('currency_id');

        if (companyCurreny !== selectedCurrency) form.setValue('amount_loc', Number((value * rate).toFixed(2)));
        else form.setValue('amount_loc', Number(value.toFixed(2)));
      },
    },
    {
      id: 'amount_loc',
