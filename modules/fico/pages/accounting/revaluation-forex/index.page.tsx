...
const columns = [
  {
    ...
    render: (value, row) => (
      <Text
        ...
        onClick={() => Router.push(`${Router.pathname}/${row.document_number}`)}
        clickable
        underLineOnHover
      >
        {value}
...
const AccrualJournal = () => {
  ...
  const submitRevaluationForex = service.submit({
    onSuccess: () => {
      setModals({
  ...
  const onSubmitSelection = () => {
    setModals({
      ...
      confirmation: {
        ...
        onOk: () => submitRevaluationForex.mutate(selectedRowKeys),
      },
    });
  };
  ...
  return (
    <>
      <Col>
        <Text variant="h4">Revaluation Forex</Text>
        <Spacer size={20} />
        <Card>
