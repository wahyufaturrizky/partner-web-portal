...
import { AccrualJournalFields } from './form';
...
export const FormGLAccount = (props: IForm<AccrualJournalFields>) => {
  const { form: formAccrualJournal, type } = props;
  ...
  useEffect(() => {
    // Default Value
    if (type === 'create') {
      formAccrualJournal.setValue('company_code', 'PP01');
    }
  }, []);

  useEffect(() => {
    preparePayload(formAccrualJournal.getValues('items'));
  }, [formAccrualJournal.getValues('items')]);

  const { getValues: getValuesGeneralJournal, setValue: setValueGeneralJournal } = formAccrualJournal;
  const {
    handleSubmit,
    getValues,
    setValue,
  ...
  const columns = [
    ...
    { title: 'D/C', dataIndex: 'debit_type', render: (value) => (value === 'true' || value === true ? 'Debit' : 'Credit') },
    { title: 'Amount In Doc. Currency', dataIndex: 'ammount', render: (value) => currencyFormater(value) },
    { title: 'Ammount In Local Currency', dataIndex: 'local_currency', render: (value) => (value ? currencyFormater(value) : '-') },
    { title: 'Tax Code', dataIndex: 'tax' },
    { title: 'Asset Number', dataIndex: 'asset_number' },
    // { title: 'Asset Class', dataIndex: 'asset_class', render: (value) => DS_ASSET_CLASS.find((v) => v.id === value)?.name },
  ...
  const preparePayload = (newItems) => {
    ...
    setValueGeneralJournal('dc', newItems.map((item) => (item.debit_type === 'true' ? 'debit' : 'credit')));
    ...
    setValueGeneralJournal('asset_numbers', newItems.map((item) => item.asset_number ?? null));
    // setValueGeneralJournal('asset_class', newItems.map((item) => item.text ?? null));
    setValueGeneralJournal('order_list', newItems.map((item) => item.orders ?? null));

    const balance = newItems.reduce((accumulator, item) => {
      if (item.debit_type === 'true' || item.debit_type === true) return accumulator + item.ammount;
  ...
  const queryMasterGL = useQueryMasterGeneralLedger({
    query: { search: getSearchValue('account_number'), company_code: formAccrualJournal.getValues('company_code') },
  });
  ...
  const queryMasterCostCenter = useQueryMasterCostCenter({
    query: { search: getSearchValue('cost_center'), company_code: formAccrualJournal.getValues('company_code') },
  });
  const queryMasterProfitCenter = useQueryMasterProfitCenter({
    query: { search: getSearchValue('profit_center'), company_code: formAccrualJournal.getValues('company_code') },
  });
  ...
  const fields: IField<GLAccountFields>[] = [
    {
      ...
      datasources: queryMasterGL.data?.map((v) => ({ id: v.id, value: v.id, label: v.text })),
      onChange: ({ target }) => {
    ...
    },
    {
      ...
      label: 'D/C',
      placeholder: 'Select',
      datasources: DS_DEBIT_CREDIT.map((v) => ({ id: v.id === 'debit' ? 'true' : 'false', value: v.name })),
      validation: { required: '* required' },
    },
    {
      id: 'ammount',
      ...
      onChange: ({ target }) => {
        const { value } = target;
        const rate = formAccrualJournal.getValues('rate');
        const companyCurreny = formAccrualJournal.getValues('company_currency');
        const selectedCurrency = formAccrualJournal.getValues('currency');
