...
import { FormAccrualJournal, AccrualJournalFields } from './form';
...
function AccrualJournal() {
  ...
  const getAccrualJournalByID = service.getByID({
    id,
    onSuccess: (res) => {
  ...
  const updateAccrualJournal = service.update({
    id,
    onSuccess: (res) => {
  ...
  const getPayload = (data: AccrualJournalFields) => ({
    ...data,
    company_code: 'PP01',
    document_date: moment(data.document_date, 'DD/MM/YYYY').format('YYYY-MM-DD'),
    posting_date: moment(data.posting_date, 'DD/MM/YYYY').format('YYYY-MM-DD'),
    fiscal_year: moment(data.document_date, 'DD/MM/YYYY').format('YYYY'),
    accrue_period: Number(data.accrue_period),
  });
  const doSubmit = (data) => updateAccrualJournal.mutate(getPayload({ ...data, state: 'submit' }));
  const doSave = (data) => updateAccrualJournal.mutate(getPayload({ ...data, state: 'draft' }));

  if (getAccrualJournalByID.isLoading) {
    return <FormSkeleton />;
  }
    ...
    <>
      <Col>
        <Row gap="4px" alignItems="center">
          <ArrowLeftIcon onClick={() => Router.push('/accounting/accrual-journal')} style={{ cursor: 'pointer' }} />
          <Text variant="h4">{`Document Number ${getValues('document_number') || id || 'Loading..'}`}</Text>
        </Row>
        <Card padding="20px">
          <Row justifyContent="space-between" alignItems="center">
            ...
            <Row>
              <Row gap="16px">
                ...
                >
                  {updateAccrualJournal.isLoading ? 'Loading...' : 'Save as Draft'}
                </Button>
                <Button
                  size="big"
                  variant="primary"
                ...
                >
                  {updateAccrualJournal.isLoading ? 'Loading...' : 'Submit'}
                </Button>
              </Row>
            </Row>
          </Row>
        ...
        <Spacer size={20} />
        <FormAccrualJournal form={form} type="update" />
      </Col>
      {modals?.confirmation && (
        <ModalConfirmation
