...
const OptionText = styled.div<{disabled}>`
...
const InputSearch = styled(Input)<{tokenseparators}>``;

const InputSelect = styled(Select)<{rounded, error}>`
  ...
  .ant-select-selector {
    min-height: 48px !important;
    background: #ffffff;
    border: 1px solid ${(p) => (p.error ? p.theme.red.regular : p.theme.grey.light)};
    box-sizing: border-box !important;
    border-radius: ${(p) => (p.rounded ? '64px' : '8px')} !important;
    display: flex;
    align-items: center;
