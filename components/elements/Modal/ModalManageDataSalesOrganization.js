...
export const ModalManageDataEdit = ({
	...
	const onAddStructure = () => {
		...
		const newData = {
			...
			hirarcies: [
				{ name },
				...(parentData && parentData.hirarcies)
			],
  ...
  const paginateAllStructure = tempAllStructure?.slice(5 * (pageManageData - 1), 6 * pageManageData) || [];

	paginateAllStructure?.forEach((data) => {
		let newData = data?.hirarcies?.reduce((obj, item, index) => ({
			...obj,
      [index+1]: item.name
		}), {});
		newData.key = data?.id;
	...
	const deleteDataInStructure = () => {
		...
		updateStructureClone.add = updateStructureClone.add.filter(data => !isNaNRow.includes(data.id))
