...
async function downloadStructure(structure: any, level: any) {
...
export const ModalManageData = ({
	...
	const updateParentTable = (obj: any, allArray: any, level: any) => {
		...
		if (obj.parent) {
			obj.parent = parentArray.find((arr: any) => arr?.id === obj?.parent);
	...
	if (tempAllStructure.length > 0 && tempAllStructure.length === level) {
		tableData = mapParent(tempAllStructure).filter((data: any) =>
	...
	const dataTable: any = [];
	paginateTableData.forEach((data: any) => {
		dataTable.push({
			key: data.id,
			1: data.name,
	...
	if (parentDataStructure?.length > 0) {
		parentStructure = parentDataStructure
			...
			?.filter((data: any) => data?.value?.toLowerCase()?.includes(searchParent?.toLowerCase()));
	}
			...
			data.forEach((data: any) => {
				...
				if (
					...
					const newSingleData: any = {
						id: self?.crypto?.randomUUID(),
						name: currentData,
						parent: parentDataStructure.find((parent: any) => parent.name.toLowerCase() === currentParentData.toLowerCase()).id,
					};
						...
						{level > 1 && (
							<Row height="98px" width="100%" noWrap>
								<Dropdown
									...
									placeholder={"Select"}
									onSearch={(search: any) => setSearchParent(search)}
									error={errors?.parent}
