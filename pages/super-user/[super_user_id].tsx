...
const CreateSuperUser: any = () => {
	...
	const { mutate: deletePermission } = useDeleteSuperUser({
		options: {
			onSuccess: () => {
	...
	const onSubmit = (data) => updateSuperUser(data);
				...
				<Col>
					...
					<Card padding="20px">
						<Row justifyContent="space-between" alignItems="center" nowrap>
							<Dropdown
								...
								noSearch
								defaultValue={superUser?.activeStatus}
							/>
							<Row>
								<Row gap="16px">
									<Button
					...
					{modalDelete.open && (
						<ModalDeleteConfirmation
							...
							onOk={() => deletePermission({ ids: [super_user_id] })}
							itemTitle={superUser?.fullName}
						/>
					)}
				</Col>
			)}
