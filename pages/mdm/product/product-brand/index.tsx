...
const downloadFile = (params: any) => mdmDownloadService("/product-brand/template/download", { params }).then((res) => {
...
const ProductBrandMDM = () => {
  ...
  const { mutate: createProductBrandMDM, isLoading: isLoadingCreateProductBrandMDM } = useCreateProductBrandMDM({
    options: {
      onSuccess: () => {
  ...
  const { mutate: updateProductBrandMDM, isLoading: isLoadingUpdateProductBrandMDM } = useUpdateProductBrandMDM({
  ...
  const { data: dataParentProductBrandMDM, isLoading: isLoadingParentProductBrandMDM } = useParentProductBrandMDM({
    id: `${modalProductBrandForm.data?.id ?? 0}/KSNI`,
  });

  const { mutate: deleteProductBrandMDM, isLoading: isLoadingDeleteProductBrandMDM } = useDeleteProductBrandMDM({
    options: {
      onSuccess: () => {
              ...
              >
                {modalProductBrandForm.typeForm === "create" ? (
                  <Button
                    ...
                    key="submit"
                    type="primary"
                    onClick={() => setModalProductBrandForm({ open: false, data: {}, typeForm: "" })}
                  >
