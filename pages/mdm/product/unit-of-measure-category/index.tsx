...
const downloadFile = (params: any) => mdmDownloadService("/uom-category/download", { params }).then((res) => {
...
const UOMCategory = () => {
  ...
  const { mutate: uploadFileUomCategory, isLoading: isLoadingUploadFileUomCategory } = useUploadFileUOMCategory({
    options: {
      onSuccess: () => {
        queryClient.invalidateQueries(["uom-categories"]);
        setShowUpload(false);
