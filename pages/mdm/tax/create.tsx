...
import {
  useCountryTaxInfiniteLists, useCreateTax, useDeletTax, useTaxInfiniteLists, useUpdateTax,
...
const TaxCreate = () => {
  ...
  const handleNewTax = (tax: any) => {
    ...
    const n = taxData ? taxData.length + 1 : 1;
    const newTax = {
      key: n,
      country_id: tax.country_id,
      name: tax.name,
    ...
    if (!taxData) {
      setTaxData([newTax]);
    } else if (!taxData?.find((e) => e.name === newTax.name)) {
