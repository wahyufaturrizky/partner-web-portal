...
const CostCenterCreate = () => {
    ...
    options: {
      onSuccess: (data: any) => {
        ...
        setNewCostCenterTable([
          {
            actual_primary_cost: data?.actualPrimaryCost === "YES",
            plant_primary_cost: data?.plantPrimaryCost === "YES",
            actual_secondary_cost: data?.actualSecondaryCost === "YES",
            plant_secondary_cost: data?.plantSecondaryCost === "YES",
            actual_revenues: data?.actualRevenues === "YES",
            plant_revenues: data?.plantRevenues === "YES",
            commitment_update: data?.commitmentUpdate === "YES",
          },
        ]);
      },
  ...
  const onSave = (data: CostCenterSave) => {
    const newCostCenter = {
      language: languageStatus
        ? costCenterData?.language
        : data?.language
          ? data?.language
          : costCenterData?.language,
      currency: currencyStatus
        ? costCenterData?.currency
        : data?.currency
          ? data?.currency
          : costCenterData?.currency,
      profit_center_id: data?.profit_center_id
        ? data?.profit_center_id
        : costCenterData?.profitCenterId,
      ...
      valid_from: data?.valid_from
        ? data?.valid_from
        : moment(costCenterData?.validFrom).utc().format("DD/MM/YYYY"),
      valid_to: data?.valid_to
        ? data?.valid_to
        : moment(costCenterData?.validTo).utc().format("DD/MM/YYYY"),
  ...
  const columns = [
    {
      ...
      render: (status: boolean, rowKey: any) => (
        <Checkbox
          checked={status}
          onChange={() => selectOneCheckbox(status, "actual_primary_cost")}
        />
      ),
    },
    {
      ...
      render: (status: boolean, rowKey: any) => (
        <Checkbox
          checked={status}
          onChange={() => selectOneCheckbox(status, "plant_primary_cost")}
        />
      ),
    },
    {
      ...
      render: (status: boolean, rowKey: any) => (
        <Checkbox
          checked={status}
          onChange={() => selectOneCheckbox(status, "actual_secondary_cost")}
        />
      ),
    },
    {
      ...
      render: (status: boolean, rowKey: any) => (
        <Checkbox
          checked={status}
          onChange={() => selectOneCheckbox(status, "plant_secondary_cost")}
        />
      ),
    },
    {
    ...
    },
    {
      ...
      render: (status: boolean, rowKey: any) => (
        <Checkbox
          checked={status}
          onChange={() => selectOneCheckbox(status, "commitment_update")}
        />
      ),
    },
  ];
  ...
  const selectOneCheckbox = (checked: boolean, name: string) => {
    const newCheckbox = newCostCenterTable[0];
    switch (name) {
  ...
  const disabledDate: RangePickerProps["disabledDate"] = (current: number) => current <= moment().startOf("day");
    ...
    <Col>
      ...
      <Card>
                ...
                render={({ field: { onChange } }) => (
                  <>
                    ...
                    <FormSelect
                      ...
                      items={
                        isFetchingLanguagesData && !isFetchingMoreLanguagesData ? [] : listLanguages
                      }
                      onChange={(value: any) => {
                        onChange(value);
                      }}
              ...
              render={({ field: { onChange }, fieldState: { error } }) => (
                <>
                  ...
                  <FormSelect
                    ...
                    withSearch
                    defaultValue={
                      listProfitCenter.filter(
                        (profitCenter) => profitCenter.value === costCenterData?.profitCenterId,
                      )[0]?.label
                    }
                    isLoading={isLoadingProfitCenterData}
                    items={
        ...
        <Col>
          ...
          <Col gap="60px">
            <Table columns={columns} data={newCostCenterTable} bordered rowKey="id" />
          </Col>
        </Col>
      </Card>
    </Col>
