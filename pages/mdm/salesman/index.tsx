...
const downloadFile = (params: any) => mdmDownloadService("/salesman/template/download", { params }).then((res) => {
...
export default function Salesman() {
  ...
  const columns = [
    ...
    },
    {
      title: "Action",
      render: ({
        id, statusText, idCard, name, division,
      }: any) => listPermission?.filter((data: any) => data.viewTypes[0]?.viewType.name === "View").length
          > 0 && (
          <Button
            size="small"
            variant="tertiary"
