...
const DetailMenuList: any = () => {
  ...
  const [parentInit, setParentInit] = useState("");
  ...
  const { mutate: updatePurchaseOrganization, isLoading: isLoadingUpdatePurchaseOrganization } = useUpdatePurchaseOrganizationMDM({
    options: {
      onSuccess: (data) => {
        if (data) {
          setIsLoading(false);
