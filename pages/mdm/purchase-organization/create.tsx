...
const CreateMenuList: any = () => {
  ...
  const { mutate: reqBodyFilterListPermission, data: fieldsTablePermissionFilter } = useFilterListPermissions({
    options: {
      onSuccess: (data) => {
        paginationTablePermission.setTotalItems(data.totalRow);
        setLoading(false);
  ...
  const { data: dataParentPurchaseOrganization, isLoading: isLoadingParentPurchaseOrganization } = useParentPurchaseOrganizationMDM({
    id: `${0}/${companyCode}`,
  });
  ...
  const { mutate: createPurchaseOrganization, isLoading: isLoadingurchaseOrganization } = useCreatePurchaseOrganizationMDM({
    options: {
      onSuccess: (data) => {
        if (data) {
          setLoading(false);
  ...
  const handleCreateMenuList = () => {
    ...
    dataAssociatedPermissionsField?.forEach(
      (permission, i) => (product_categories
          ...
          <>
            ...
            <Row alignItems="flex-end" justifyContent="space-between">
              <Search
                ...
                onChange={(e) => (titleModal === "Product Category"
