...
import {
  useBranchParent, useCalendarDetail, useCalendarInfiniteLists, useCreateBranch, useTimezoneInfiniteLists,
  ...
  const {
    ...
    options: {
      onSuccess: (data: any) => {
        ...
        setListCalendar(data?.pages[0]?.rows?.map((calendar: { calendarName: string; id: number; }) => ({ label: calendar.calendarName, value: calendar.id })));
      },
      getNextPageParam: (_lastPage: any, pages: any) => {
