...
const CreatePricingStructure: any = () => {
  ...
  const [totalRowsSalesOrganizationInfiniteList, setTotalRowsSalesOrganizationInfiniteList] = useState(0);
  const [totalRowsPricingStructureInfiniteList, setTotalRowsPricingStructureInfiniteList] = useState(0);
  ...
  const [selectedRowKeysCopyFromPriceStructure, setSelectedRowKeysCopyFromPriceStructure] = useState([]);
  ...
  const { mutate: pricingStructure, isLoading: isLoadingPricingStructure } = useCreatePricingStructureList({
    options: {
      onSuccess: () => {
        router.back();
      },
  ...
  const { mutate: pricingStructureDraft, isLoading: isLoadingPricingStructureDraft } = useCreatePricingStructureDraftList({
    options: {
      onSuccess: () => {
        router.back();
      },
          ...
          action: (
            <div style={{ display: "flex", justifyContent: "left" }}>
              <Button
                size="small"
                onClick={() => {
                  router.push(`/mdm/product-list/${element.productId}`);
                }}
                variant="tertiary"
              >
                View Detail
  ...
  const {
    ...
    options: {
      onSuccess: () => {},
      select: (data: any) => data.map((dataHirarcy: any) => ({
        ...dataHirarcy,
        key: dataHirarcy.id,
      })),
      enabled:
  ...
  const { data: pricingStructureLists, isLoading: isLoadingPricingStructureList } = usePricingStructureLists({
    options: {
      ...
      select: (data: any) => data.rows.map((subData: any) => ({
        ...subData,
        key: subData.id,
  ...
  const {
    ...
    options: {
      onSuccess: (data: any) => {
        ...
        const mappedData = data?.pages?.map((group: any) => group.salesOrganizationStructures?.map((element: any) => ({
          ...element,
          value: element.id,
          label: element.name,
        })));
  ...
  const onSubmit = (dataSubmit: any) => pricingStructure({
  ...
  const onSubmitDraft = (dataDraft: any) => pricingStructureDraft({
  ...
  const handleSelectedField = (data: any) => {
    ...
    if (typeForm === "Add Products") {
      const tempProductsSelected: any = [];
                ...
                <Col width="40%">
                  <Input
                    ...
                    error={
                          errors?.product_selected?.[
                            manageByZone?.data?.indexExpandedRowRenderProductSelected
                          ]?.distribution_channel?.[manageByZone?.data?.indexDistChannel]?.level?.[
                            indexLevel
                          ]?.cost?.message
                        }
                    required
...
const Card = styled.div`
  ...
  background-attachment: ${(p: any) => (p.backgroundAttachment ? p.backgroundAttachment : undefined)};
`;
