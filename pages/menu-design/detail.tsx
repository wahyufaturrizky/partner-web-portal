...
} from "../../hooks/menu-design/useMenuDesign";
...
const DetailMenuDesignList: any = () => {
	...
	let filterByIdParentAndGroup = [];

	filterByIdParentAndGroup = dataMenuDesignById?.menuDesignToModules?.map((mapping) => {
		if (mapping.isParent) {
			delete mapping.menuDesignToMenus;
			return {
				...mapping,
				childrenMainMenu: dataMenuDesignById?.menuDesignToModules?.filter(
					(filtering) => filtering.group === mapping.group && !filtering.isParent
				),
			};
		}
	});
	...
	const handleUpdateMenuDesign = () => {
		...
		const mappingMpdules = filterByIdParentAndGroup
			?.filter((filtering) => filtering !== undefined)
			.map((data) =>
				data.childrenMainMenu.map((data) => ({ id: data.id, module_index: data.moduleIndex }))
			)
			.filter((filtering) => filtering.length !== 0)
			.map((data) => data[0]);

		const mappingMenus = filterByIdParentAndGroup
			?.filter((filtering) => filtering !== undefined)
			.map((data) =>
				data.childrenMainMenu.map((data) =>
					data.menuDesignToMenus.map((data) => ({ id: data.id, menu_index: data.menuIndex }))
				)
			)
			.filter((filtering) => filtering.length !== 0)
			.map((data) => data[0])
	...
	const onSelectChangeTableModuleConfig = (value) => {
	...
	const onSelectChangeTableMenuLists = (value) => {
	...
	const onCheckTree = (checkedKeys, info) => {
		setRemoveMenu(
			info.checkedNodes
				.filter((filtering) => "children" in filtering === false)
