...
const UserConfigUser: any = () => {
      ...
      select: (data: any) => {
        const mappedData = data?.rows?.map((element: any) => ({
          ...
          action: (
            <Button
              size="small"
              onClick={() => router.push(`/config/user-config/${element.id}`)}
              variant="tertiary"
            >
  ...
  const { mutate: uploadFileUserConfig, isLoading: isLoadingUploadFileTrainingType } = useUploadFileUserConfig({
    options: {
      onSuccess: () => {
  ...
  const allowPermissionToShow = listPermission?.filter((data: any) => permissionUserList.role[dataUserPermission?.role?.name]?.component.includes(data.name));
  ...
  const downloadFile = (params: any) => clientDownloadService("/partner-user/download", { params }).then((res) => {
            ...
            <Row gap="16px">
              ...
              {listPermission?.filter((x: any) => x.viewTypes[0]?.viewType.name === "Create")
                ...
                <Button
                  size="big"
                  variant="primary"
                  onClick={() => router.push("/config/user-config/create")}
                >
