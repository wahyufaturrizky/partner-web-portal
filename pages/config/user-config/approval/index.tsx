...
const UserConfigApproval: any = () => {
  ...
  const { mutate: deleteFields, isLoading: isLoadingDeleteProcessList } = useDeletePartnerConfigApprovalList({
    options: {
      onSuccess: () => {
        refetchFields();
  ...
  fields?.rows?.map((field: any) => {
    data.push({
      ...
      action: (
        <Button
          size="small"
          onClick={() => Router.push(`/config/user-config/approval/${field.id}`)}
          variant="tertiary"
        >
            ...
            <Row alignItems="center">
              ...
              <Dropdown
                ...
                items={
                  ...
                  && dataFieldsProcessLists?.rows.map((data: any) => ({
                    id: data.id,
                    value: data.name,
                  }))
                }
            ...
            <Row gap="16px">
              ...
              {listPermission?.filter((x: any) => x.viewTypes[0]?.viewType.name === "Create")
                ...
                <Button
                  size="big"
                  variant="primary"
                  onClick={() => Router.push("/config/user-config/approval/create")}
                >
