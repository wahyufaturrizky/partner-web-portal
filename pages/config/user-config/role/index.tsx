...
const UserConfigRole: any = () => {
  ...
  const allowPermissionToShow = listPermission?.filter((data: any) => permissionRoleList.role[dataUserPermission?.role?.name]?.component.includes(data.name));
      ...
      action: (
        <div style={{ display: "flex", justifyContent: "left" }}>
          <Button
            size="small"
            onClick={() => {
              router.push(`/config/user-config/role/${field.id}`);
            }}
            variant="tertiary"
          >
            ...
            <Row gap="16px">
              {allowPermissionToShow?.some((el: any) => el.name === "Create Role List") && (
                <Button
                  ...
                  variant="primary"
                  onClick={() => {
                    router.push("/config/user-config/role/create");
                  }}
                >
