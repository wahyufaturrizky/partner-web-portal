...
import {
  Text, Col, Row, Spacer, Dropdown, Button, Accordion, Input, FormSelect, EmptyState,
} from "pink-lava-ui";
  ...
  const {
    ...
    options: {
      onSuccess: (data: any) => {
        ...
        const mappedData = data?.pages?.map((group: any) => group?.rows?.map((employee: { nik: string; name: string; id: number; languages: string }) => ({
