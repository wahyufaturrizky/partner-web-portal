...
const UpdateUserConfig: any = () => {
  ...
  const {
    ...
    options: {
      onSuccess: (data: any) => {
        ...
        const mappedData = data?.pages?.map((group: any) => group?.rows?.map((timezone: { utc: string; id: number; name: string }) => ({
          label: timezone.utc,
          value: timezone.name,
        })));
  ...
  const {
    ...
    options: {
      onSuccess: (data: any) => {
        ...
        const mappedData = data?.pages?.map((group: any) => group?.rows?.map(
          (employee: { nik: string; name: string; id: number; languages: string }) => ({
            label: `${employee.nik} - ${employee.name}`,
            value: employee.id,
  ...
  const active_status = [
    {
      id: "ACTIVE",
      value: `<div key="1" style="color:green;">${lang[t].userList.create.template.button.active}</div>`,
    },
    {
      id: "INACTIVE",
      value: `<div key="2" style="color:red;">${lang[t].userList.create.template.button.inActive}</div>`,
    },
  ];
  ...
  return (
    <Col>
      ...
      <Accordion style={{ display: "relative" }} id="area">
        <Accordion.Item key={1}>
          <Accordion.Header variant="blue">
            {lang[t].userList.create.template.accordion.employeeInformation}
          </Accordion.Header>
          <Accordion.Body>
                  ...
                  render={({ field: { onChange } }) => (
                    <>
                      ...
                      <FormSelect
                        ...
                        items={isFetchingEmployee && !isFetchingMoreEmployee ? [] : listEmployee}
                        onChange={(value: any) => {
                          onChange(value);
      ...
      <Accordion style={{ display: "relative" }} id="area2">
        <Accordion.Item key={1}>
          <Accordion.Header variant="blue">
            {lang[t].userList.create.template.accordion.generalInformation}
          </Accordion.Header>
          <Accordion.Body>
