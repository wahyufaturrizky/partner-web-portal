...
const CreateRole: any = () => {
	...
	const { data: role, isLoading } = usePartnerConfigRole({
		role_id,
		options: {
			onSuccess: (data) => {
				setValue("name", data.name);
				setValue("activeStatus", data.activeStatus ?? "N");
				setPermissions(data.partnerRoleToPermissions.map((permission) => permission.permissionId));
			},
		},
	});
	...
	const { mutate: deletePermission } = useDeletePartnerConfigRole({
		options: {
			onSuccess: () => {
	...
	const { mutate: updateRole } = useUpdatePartnerConfigRole({
		role_id,
		options: {
			onSuccess: () => {
				Router.push("/partner-config-role-list");
        ...
        <Alert><Text variant="subtitle2" color="white">“General” Associated Menu must be filled.</Text></Alert> */}
			...
			{modalDelete.open && (
				<ModalDeleteConfirmation
					...
					onOk={() => deletePermission({ ids: [role_id] })}
					itemTitle="Test"
				/>
			)}
		</>
