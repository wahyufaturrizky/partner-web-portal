...
const ProductOptionCreate = () => {
  ...
  const { mutate: createProductOption, isLoading: isLoadingCreateProductOption } =
  ...
  const handleAddItem = (data) => {
    if (modalChannelForm.typeForm === "edit") {
      let tempEdit = dataItem.map((mapDataItem) => {
        ...
        return (
          <Row gap="16px" alignItems="center" nowrap>
            ...
            <Col>
              <ICDelete
                onClick={() =>
                  setDataItem(dataItem.filter((filtering) => filtering.id !== record.id))
                }
              />
            </Col>
          </Row>
