...
import { ModalManageData } from "../../components/modals/ModalManageData";
import { useMdmCurrency } from "../../hooks/mdm/currency/useMdmCurrency";
import _ from "lodash";
import { useMdmCreateCountry } from "../../hooks/mdm/country/useMdmCountry";
import * as ExcelJS from "exceljs/dist/exceljs.min.js";
import { saveAs } from "file-saver";

async function downloadStructure(countryStructure) {
...
const CreateConfig = () => {
	...
	const onSubmit = () => {
		let newAllStructure = _.clone(allStructure);
		if (newAllStructure.length > 0) {
			const allStructureZero =
				allStructure?.[0]?.map((data) => ({
					...data,
					parent: 0,
				})) || [];
			newAllStructure[0] = allStructureZero;
		}
		const newStructure = countryStructure.map((country, index) => {
			return {
				...country,
	...
	const recursivelyDeleteStructure = (structure, allStructure, deletedIds, level) => {
		const parentDeletedIds = [];
		deletedIds.forEach(id => {
			const findParentId =  structure.find(structure => structure.parent === id);
			parentDeletedIds.push(findParentId.id);
			allStructure[level] = structure.filter(structure => structure.parent !== id)
		})
	...
	const onAddToStructure = (tempAllStructure, level) => {
		if(!tempAllStructure) {
			setShowManageData({ visible: false });
			return;
		};

		let newStructure = _.cloneDeep(allStructure);
		// recursively delete
		if(newStructure[level] && !_.isEqual(newStructure[level-1], tempAllStructure)){
			const deletedIds = [];
			const tempAllStructureIds = tempAllStructure.map(temp => temp.id);
			newStructure[level-1].forEach(structure => {
			...
			const parentDeletedIds = [];
			deletedIds.forEach(id => {
				const findParentId =  newStructure[level].find(structure => structure.parent === id);
				parentDeletedIds.push(findParentId.id);
				newStructure[level] = newStructure[level].filter(structure => structure.parent !== id)
			})
