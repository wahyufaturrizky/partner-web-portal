...
const MenuTemplateConfigRoleList: any = () => {
	...
	const { mutate: deleteFields, isLoading: isLoadingDeleteProcessList } = useDeleteTemplateRoleList(
		{
			options: {
				onSuccess: () => {
					refetchFields();
		...
		data.push({
			...
			action: (
				<Button
					...
					onClick={() =>
						Router.push({
							pathname: "/template-config/role/detail",
							query: field,
						})
					}
					variant="tertiary"
					...
					<Row justifyContent="space-between">
						...
						<Row gap="16px">
							...
							<Button
								size="big"
								variant={"primary"}
								onClick={() => Router.push("/template-config/role/create")}
							>
								Create
							</Button>
						</Row>
...
export default MenuTemplateConfigRoleList;
