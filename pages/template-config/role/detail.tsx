...
} from "../../../hooks/template-role/useTemplateRole";

export interface ConfigModuleList {}

const DetailMenuTemplateConfigRoleList: any = () => {
	...
	const { isLoading: isLoadingTemplateConfigRole } = useTemplateRoleList({
		options: {
			onSuccess: (data) => {
				let tempDataTemplateFieldRole = [];

				data.templateRoleToRoles.map((data) => {
					tempDataTemplateFieldRole.push({
						key: data.userRole.id,
						template_role_field: data.userRole.name,
						template_role_field_id: data.userRole.id,
						template_role_field_key: data.userRole.id,
					});
				});

				setDataListStatus(data.status === "PUBLISH" ? 2 : 1);
				...
				setDataTemplateFieldRole(tempDataTemplateFieldRole);

				onSelectChangeTableTemplateField(tempDataTemplateFieldRole.map((data) => data.key));
			},
		},
		template_role_list_id: Router.query.id,
	});
	...
	const { mutate: updateTemplateRoleList, isLoading: isLoadingUpdateTemplateRoleList } =
		useUpdateTemplateRoleList({
			options: {
				onSuccess: (data) => {
					if (data) {
						window.alert("Template config role updated successfully");
						Router.back();
					}
				},
			...
			templateRoleListId: Router.query.id,
		});
					...
					<Row justifyContent="space-between" alignItems="center" nowrap>
						{isLoadingTemplateConfigRole ? (
							...
							<Dropdown
								...
								defaultValue={dataListStatus === 1 ? "Draft" : "Published"}
								noSearch
							/>
						)}
						...
						<Row gap="16px">
							...
							<Button size="big" variant={"primary"} onClick={handleCreateProcessList}>
								{isLoadingUpdateTemplateRoleList ? "loading..." : "Save"}
							</Button>
						</Row>
					</Row>
				</Card>
				...
				<Accordion>
					<Accordion.Item key={1}>
						...
						<Accordion.Body>
							...
							<Spacer size={10} />
							<Table
								loading={isLoadingTemplateConfigRole}
								columns={columns}
...
export default DetailMenuTemplateConfigRoleList;
